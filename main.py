...
def init_database():
    ...
    # --- Create blog_post table if not exists ---
    try:
        db.session.execute(db.text("""
        ...
        db.session.execute(db.text(
            'CREATE INDEX IF NOT EXISTS idx_blog_published ON blog_post(published)'
        ))
        db.session.commit()
        print("✅ Blog table and indexes ensured")
    except Exception as e:
        print("⚠️ Blog table creation skipped:", e)
    ...
    try:
        result = db.session.execute(db.text('SELECT COUNT(*) FROM blog_post'))
        count = result.scalar()
        if count == 0:
            ...
            blog_posts = [
                {
                    ...
                    'meta_desc': 'Compare 40+ bad credit mortgage lenders that accept credit scores from 300. Bank statement, guarantor & specialist options. Free eligibility checker.',
                    'content': bad_credit_content,
                    'category': 'Bad Credit',
                ...
                },
                {
                    ...
                    'meta_desc': 'Get a mortgage without payslips! 7 bank statement lenders accept self-employed, cash workers, gig economy. Compare rates from 6.35%.',
                    'content': self_employed_content,
                ...
                },
                {
                    ...
                    'meta_desc': 'Yes, you CAN get a mortgage with 400 credit score! 7 lenders accept 400-450 scores. Guarantor, credit union & bank statement options from 5.30%.',
                    'content': credit_400_content,
                    'category': 'Bad Credit',
            ...
            for post in blog_posts:
                db.session.execute(db.text('''
                    ...
                    VALUES (:slug, :title, :meta_desc, :content, :category, :keywords, 1, 0)
                '''), post)

            db.session.commit()
            print(f'✅ Seeded {len(blog_posts)} blog posts')
    except Exception as e:
        print(f'⚠️ Blog seeding skipped: {e}')
